...
describe('recurring service', () => {
  ...
  it('advances an item when its wallet is archived', async () => {
    mocks.repo.findDue.mockResolvedValue([
      {
        ...
        frequency: 'daily',
        anchorDay: null,
        nextRunOn: '2026-08-22',
      },
    ])
    mocks.users.getProfile.mockResolvedValue({ timezone: 'Asia/Ho_Chi_Minh' })
    mocks.transactions.createTransaction.mockRejectedValue(
      new AppError('WALLET_ARCHIVED', 'wallet_archived'),
    )

    await recurringService.runDue({} as never, '2026-08-22')

    expect(mocks.notifications.create).not.toHaveBeenCalled()
    expect(mocks.repo.advance).toHaveBeenCalledWith({}, 'recurring-archived', '2026-08-23')
  })

  it('does not advance an item after an unexpected materialization failure', async () => {
    mocks.repo.findDue.mockResolvedValue([
      {
        id: 'recurring-failed',
        userId: 'user-1',
    ...
    mocks.users.getProfile.mockResolvedValue({ timezone: 'Asia/Ho_Chi_Minh' })
    mocks.transactions.createTransaction.mockRejectedValue(new Error('database unavailable'))

    await recurringService.runDue({} as never, '2026-08-22')

    expect(mocks.repo.advance).not.toHaveBeenCalled()
  })
})
